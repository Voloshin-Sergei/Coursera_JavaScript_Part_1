...
function removePhoneContact(PhoneRemove) { // Функция удаляет номер из телефонной книги
    ...
    if (phoneBook[key].includes(phoneRemove)) { // Проверяем содержит ли значения свойства объекта элемент PhoneRemove (зависимости от этого вернется true или false)
    if (phoneBook[key].length > 1) { // Если значений в свойстве объекта больше чем 1 элемент
        phoneBook[key].splice((phoneBook[key].indexOf(phoneRemove)),1); // Методом splice() изменим содержимое в свойстве объекта (1й параметр вернувщийся идекс, 2й параметр колличество удаления элементов)
    } // Методом indexOf() возвращаем первый индекс, по которому данный элемент может быть найден в свойстве объекта
    else {
